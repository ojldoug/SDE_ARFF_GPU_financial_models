...
class Functions:
    ...
    def S(omega, x):
        x_proj = x @ omega
        return jnp.concatenate([jnp.cos(x_proj), jnp.sin(x_proj), jnp.ones((x.shape[0], 1))], axis=-1)
...
class ARFFTrain:
    ...
    def ARFF_one_step(key, omega, amp, x, y, delta, lambda_reg, gamma, RESAMPLING=True, METROPOLIS_TEST=True):
        ...
        # Drop the bias row so we only have 2K rows (cos & sin) for per-frequency norms
        amp_main = amp[:-1, :]  # shape (2K, D_out)
    
        # Same reshape trick as before; works for any D_out
        amp_pairs = jnp.reshape(amp_main, (-1, K))  # shape (2*D_out, K)
        amp_norm = jnp.linalg.norm(amp_pairs, axis=0)  # (K,)
    
        if RESAMPLING:
            amp_pmf = amp_norm / jnp.sum(amp_norm)
            key, subkey = random.split(key)
            omega = omega[:, random.choice(subkey, amp_norm.shape[0],
                                           shape=(K,), p=amp_pmf)]
        ...
        if METROPOLIS_TEST:
            ...
            amp_prime = ARFFTrain.get_amp(x, y, lambda_reg, omega_prime)
            amp_prime_main = amp_prime[:-1, :]
            amp_prime_pairs = jnp.reshape(amp_prime_main, (-1, K))
            amp_prime_norm = jnp.linalg.norm(amp_prime_pairs, axis=0)
    
            key, subkey = random.split(key)
            accept = (amp_prime_norm / (amp_norm + 1e-12)) ** gamma
            thresh = random.uniform(subkey, (K,))
            omega = jnp.where(accept >= thresh, omega_prime, omega)
